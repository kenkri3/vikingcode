...
import { CheckCircle2, Rocket, ExternalLink, Sparkles, X, Github } from "lucide-react";

export default function VikingCodeApp() {
  // Bruker-sesjon (starter i TRIAL-modus for å demonstrere sikkerhetsventilen)
  const [user, setUser] = useState<UserSession>({
    id: "user-kenneth-1",
    email: "kenneth@vikingcode.no",
    name: "Kenneth Glosli Kristiansen",
    plan: "TRIAL",
    tokensRemaining: 50000,
    trialPromptsUsed: 1, // 1 av 3 brukt for umiddelbar visning av telleren
    isActive: true,
  });
  ...
  const handleSendMessage = async (promptText: string, model: string) => {
    // 1. Sjekk token-grense umiddelbart
    ...
    try {
      ...
      // Oppdater brukerens tokens og trial-kvote
      setUser((prev) => ({
        ...prev,
  ...
  const handleDeployRailway = async () => {
    ...
    setIsDeploying(true);
    try {
      // Pusher først til GitHub repo slik at Railway kan importere
      ...
      // Åpne Railway direkte i ny fane
